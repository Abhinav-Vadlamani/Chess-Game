...
A simple but strong chess engine (~2000 Elo)
Adapted from https://github.com/thomasahle/sunfish
"""
...
# Board is represented as a 120 character string with padding
...
def render(i):
    """Render board index to algebraic notation"""
    rank, fil = divmod(i - A1, 10)
    return chr(fil + ord("a")) + str(-rank + 1)


def board_to_sunfish_index(row, col):
    """Convert (row, col) to Sunfish board index

    Your board: row 0 = rank 8 (black side), row 7 = rank 1 (white side)
    Sunfish: A8=21, H8=28, A1=91, H1=98
    """
    return 21 + (row * 10) + col


def sunfish_index_to_board(idx):
    """Convert Sunfish board index to (row, col)"""
    row = (idx - 21) // 10
    col = (idx - 21) % 10
    return row, col
